    ...
    if user.id == OWNER_ID:
        ...
        if user.id in SUDO_USERS:
            text += "\nЭтот человек - один из моих пользователей sudo! "\
                    "Почти такой же мощный, как мой владелец - так что следите за своими действиями."
        else:
            if user.id in SUPPORT_USERS:
                text += "\nЭтот человек - один из моих пользователей службы поддержки! "\
                        "Не совсем пользователь sudo, но все же может заблокировать легким движением руки!"
