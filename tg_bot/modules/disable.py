...
if is_module_loaded(FILENAME):
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Следующие команды в настоящее время отключены:\n{}".format(result)
    ...
    def __stats__():
        return "{} отключенные элементы в {} чатах.".format(sql.num_disabled(), sql.num_chats())
...
*Admin only:*
 - /enable <cmd name>: включить команду
 - /disable <cmd name>: отключить команду
