...
from sqlalchemy import BigInteger, Text, BigInteger
...
class Users(BASE):
    ...
    def __repr__(self):
        return "<Пользователь {} ({})>".format(self.username, self.user_id)
...
class Chats(BASE):
    ...
    def __repr__(self):
        return "<Чат {} ({})>".format(self.chat_name, self.chat_id)


class ChatMembers(BASE):
    __tablename__ = "chat_members"
    priv_chat_id = Column(Integer, primary_key=True)
    # NOTE: Использовать двойной первичный ключ вместо частного первичного ключа?
    ...
    def __repr__(self):
        return "<Пользователь чата {} ({}) в чате {} ({})>".format(self.user.username, self.user.user_id,
