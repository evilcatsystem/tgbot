...
class Warns(BASE):
    ...
    def __repr__(self):
        return "<{} предупреждения для {} в {} по причинам {}>".format(self.num_warns, self.user_id, self.chat_id, self.reasons)
...
class WarnFilters(BASE):
    ...
    def __repr__(self):
        return "<Разрешения для %s>" % self.chat_id
...
class WarnSettings(BASE):
    ...
    def __repr__(self):
        return "<{} имеет {} предупреждений.>".format(self.chat_id, self.warn_limit)
