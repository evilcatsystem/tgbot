...
class GloballyBannedUsers(BASE):
    ...
    def __repr__(self):
        return "<Забаненные пользователи {} ({})>".format(self.name, self.user_id)
...
class GbanSettings(BASE):
    ...
    def __repr__(self):
        return "<Настройки бана {} ({})>".format(self.chat_id, self.setting)
...
# Создайте идентификатор пользователя в памяти, чтобы избежать доступа к диску
__load_gbanned_userid_list()
