    ...
    if LOAD or NO_LOAD:
        ...
        if to_load:
            if not all(any(mod == module_name for module_name in all_modules) for mod in to_load):
                LOGGER.error("Недопустимые имена загрузочных имен. Выход.")
                quit(1)
        ...
        if NO_LOAD:
            LOGGER.info("Не загруженные модули: {}".format(NO_LOAD))
...
LOGGER.info("Загружаемые модули: %s", str(ALL_MODULES))
