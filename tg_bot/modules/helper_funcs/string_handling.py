...
def _selective_escape(to_parse: str) -> str:
    """
    Избежать всей недопустимой markdown

    : param to_parse: текст для экранирования
    : return: допустимая строка markdown
    """
...
def markdown_parser(txt: str, entities: Dict[MessageEntity, str] = None, offset: int = 0) -> str:
    """
    Анализируйте строку, избегая всех недопустимых объектов markdown.

    Экранирует URL-адреса, чтобы избежать искажения URL-адресов.
    Повторно добавляет любые сущности кода телеграммы, полученные из объекта сущностей.

    : param txt: текст для анализа
    : param entity: dict сущностей сообщения в тексте
    : param offset: message offset - длина команды и notename
    : return: допустимая строка markdown
    """
    if not entities:
        entities = {}
...
def extract_time(message, time_val):
    if any(time_val.endswith(unit) for unit in ('m', 'h', 'd')):
        ...
        if not time_num.isdigit():
            message.reply_text("Указана неверная сумма времени.")
            return ""
        ...
        message.reply_text("Указан недопустимый тип времени. Ожидаемые m, h или d, получили: {}".format(time_val[-1]))
        return ""
