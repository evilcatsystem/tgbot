...
def locktypes(bot: Bot, update: Update):
    update.effective_message.reply_text("\n - ".join(["Заблокировано: "] + list(LOCK_TYPES) + list(RESTRICTION_TYPES)))
    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("Заблокировано {} сообщений для всех, не являющихся администраторами!".format(args[0]))

                return "<b>{}:</b>" \
                       "\n#LOCK" \
                       "\n<b>Admin:</b> {}" \
                       "\nЗаблокировано <code>{}</code>.".format(html.escape(chat.title),
                ...
                message.reply_text("Заблокировано {} для всех, не являющихся администраторами!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#LOCK" \
                       "\n<b>Admin:</b> {}" \
                       "\nзаблокировано <code>{}</code>.".format(html.escape(chat.title),
                ...
                message.reply_text("Что вы пытаетесь заблокировать ...? Попробуйте /locktypes для списка блокируемых")

    else:
        message.reply_text("Я не администратор или у меня нет прав на удаление.")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("Разблокирован {} для всех!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                       "\nразблокировано<code>{}</code>.".format(html.escape(chat.title),
                                                            mention_html(user.id, user.first_name), args[0])
                ...
                message.reply_text("Что вы пытаетесь разблокировать ...? Попробуйте /locktypes для списка блокируемых")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("Я вижу бота, и мне сказали, чтобы они не присоединялись ... "
                                               "но я не админ!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text("Только администраторы могут добавлять ботов в этот чат! Убирайся отсюда.")
            else:
                try:
                    message.delete()
                    ...
                    if excp.message == "Message to delete not found":
                        ...
                        LOGGER.exception("Ошибка в заблокированных элементах")
        ...
        if filter(msg) and sql.is_restr_locked(chat.id, restriction) and can_delete(chat, bot.id):
            try:
                ...
                if excp.message == "Message to delete not found":
                    ...
                    LOGGER.exception("ОШИБКА в ограничениях")
            break
...
def build_lock_message(chat_id):
    ...
    if not (locks or restr):
        res = "В этом чате нет текущих блокировок."
    else:
        res = "В этом чате заблокировано"
        if locks:
            res += "\n - sticker = `{}`" \
                   "\n - audio = `{}`" \
                   "\n - voice = `{}`" \
...
*Admin only:*
 - /lock <type>: блокировать предметы определенного типа (недоступно в частном чате)
 - /unlock <type>: разблокировать предметы определенного типа (недоступно в частном чате)
 - /locks: текущий список блокировок в этом чате.

Блокировки могут использоваться для ограничения пользователей группы.
например:
Блокировка URL-адресов автоматически удалит все сообщения с URL-адресами, блокировка стикеров удалит все \
наклейки и др.
Блокировка ботов не позволит администраторам добавлять ботов в чат.
"""

__mod_name__ = "Блокировка"
